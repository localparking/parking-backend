...
class AdminController (
    ...
    fun getStorekeeperRequests(@RequestParam status: RequestStatus):
    ...
    @PostMapping("/storekeeper-requests/{userId}/approve")
    fun approveStorekeeperRequest(@PathVariable userId: Long): ResponseEntity<ResponseDto<Unit>> {
        adminService.processStorekeeperRequest(userId, true)
    ...
    @PostMapping("/storekeeper-requests/{userId}/reject")
    fun rejectStorekeeperRequest(@PathVariable userId: Long): ResponseEntity<ResponseDto<Unit>> {
        adminService.processStorekeeperRequest(userId, false)
        return ResponseEntity.ok(ResponseDto.empty(SuccessCode.OK))
    }
}